		...
		<div className='relative '>
			...
			>
				<div className='relative flex overflow-hidden items-center justify-center rounded-full w-12 h-12 transform transition-all bg-slate-700 ring-0 ring-gray-300 hover:ring-8 group-focus:ring-4 ring-opacity-30 duration-200 shadow-md'>
					<div className='flex flex-col justify-between w-5 h-5 transform transition-all duration-300 origin-center overflow-hidden'>
						<div
							className={`bg-white h-1 w-7 transform transition-all duration-300 origin-left ${isOpen ? 'translate-x-10' : '-translate-x-0'} flex w-0 ${isOpen ? 'group-focus:w-12' : 'w-12'} `}
						></div>
						<div
							className={`bg-white h-1 w-7 rounded transform transition-all duration-300 delay-75 ${isOpen ? 'translate-x-10' : '-translate-x-0'} flex w-0 ${isOpen ? 'group-focus:w-12' : 'w-12'} `}
						></div>
						<div
							className={`bg-white h-1 w-7 transform transition-all duration-300 origin-left  delay-150 ${isOpen ? 'translate-x-10' : '-translate-x-0'} flex w-0 ${isOpen ? 'group-focus:w-12' : 'w-12'} `}
						></div>
			...
			{isOpen && (
				<ul
					...
					className='absolute rounded-lg top-full right-0 mt-2 w-60 bg-white shadow-lg ring-1 ring-gray-300 transition-all duration-300'
				>
					{navItems.map(({ link, path }) =>
						link === 'Logout' ? (
							<li
								...
								className='py-3 px-5 text-start text-medium hover:bg-blue-100 hover:text-large cursor-pointer '
							>
								<NavLink
									to={path}
